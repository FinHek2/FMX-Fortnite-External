...
void FireSingle(AFortWeapon* FortWeapon)
{
	if (Settings::Misc::BulletTraces)
	{
		...
		{
			if (UKismetSystemLibrary::LineTraceSingle(GWorld, Muzzle, MaxLocation, ETraceTypeQuery::TraceTypeQuery5, false, ActorsToIgnore, EDrawDebugTrace::ForDuration, OutHit, false))
			{
				BulletImpact = OutHit.TraceEnd;
			}

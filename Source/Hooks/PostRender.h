...
void(*PostRenderOriginal)(UGameViewportClient* Viewport, UCanvas* Canvas) = nullptr;
void PostRender(UGameViewportClient* Viewport, UCanvas* Canvas)
{
	if (!Viewport || !Canvas)
		return PostRenderOriginal(Viewport, Canvas);
	Variables::ScreenSize = FVector2D(double(Variables::DebugCanvasObject->ClipX()), double(Variables::DebugCanvasObject->ClipY()));
	...
	do {
		...
		if (!GWorld) return PostRenderOriginal(Viewport, Canvas);

		UGameInstance* Gameinstance = GWorld->OwningGameInstance();
		if (!Gameinstance) return PostRenderOriginal(Viewport, Canvas);

		ULocalPlayer* LocalPlayer = Gameinstance->LocalPlayers()[0];
		if (!LocalPlayer) return PostRenderOriginal(Viewport, Canvas);

		APlayerController* PlayerController = LocalPlayer->PlayerController();
		if (!PlayerController) return PostRenderOriginal(Viewport, Canvas);
		Variables::PlayerController = PlayerController;

		APlayerCameraManager* PlayerCameraManager = PlayerController->PlayerCameraManager();
		if (!PlayerCameraManager) return PostRenderOriginal(Viewport, Canvas);

		APlayerState* PlayerState = PlayerController->PlayerState();
		if (!PlayerState) return PostRenderOriginal(Viewport, Canvas);
		...
		if (AcknowledgedPawn)
		{
			...
			if (Settings::Environment::Container)
			{
				...
				for (int i = 0; i < ContainerArray.Count; i++)
				{
					...
					if (PlayerController->ProjectWorldLocationToScreen(WorldLocation, &ScreenLocation))
					{
						...
						if (*(byte*)(ContainerActor + 0xd82) & (1 << 2)) continue; // bAlreadySearched
			...
			if (Settings::Exploits::PlayerTeleport)
			{
				...
				if (TargetMarker)
				{
					...
					FRotator WorldRotation = UKismetMathLibrary::FindLookAtRotation(Variables::CameraLocation, WorldLocation);
					AcknowledgedPawn->K2_SetActorLocationAndRotation(WorldLocation, WorldRotation);
				}
			}
			...
			if (PlayerCheck)
			{
				...
				if (Settings::Player::Wireframe)
				{
					...
					for (int mi = 0; mi < MaterialInstances.Count; mi++)
					{
						...
						*(byte*)(Material + 0x198) |= 1 << 6; // Wireframe
						*(byte*)(Material + 0x150) |= 1 << 4; // TwoSided
						*(byte*)(Material + 0x188) |= 1 << 0; // bDisableDepthTest
					}
					...
					if (USkeletalMeshComponent* Mesh = Target->Mesh())
					{
						if (AFortWeapon* CurrentWeapon = AcknowledgedPawn->CurrentWeapon())
						{
							...
							if (IsHoldingWeapon)
							{
								...
								if (!IsHoldingPickaxe)
								{
									Bones TargetBoneIndex = Bones::head;
									...
									if (Settings::Aimbot::BoneType == 1)
										...
										if (UKismetSystemLibrary::GetGameTimeInSeconds(GWorld) - LastBoneChangeTime >= 2.00)
										{
									...
									if (PlayerController->ProjectWorldLocationToScreen(TargetWorld, &TargetScreen))
									{
										if (Custom::InCircle(Variables::ScreenCenter.X, Variables::ScreenCenter.Y, AimbotFOV, TargetScreen.X, TargetScreen.Y))
										{
											...
											if (Settings::Aimbot::Triggerbot)
											{
												...
												bool TriggerbotTargeting = (Settings::Aimbot::TriggerBot::FireOnlyOnAimIn && *(byte*)(CurrentWeapon + 0xca0) & (1 << 5)) || !Settings::Aimbot::TriggerBot::FireOnlyOnAimIn;
												...
												if (TriggerbotAimkey && TriggerbotTargeting && TriggerbotProjectile)
												{
													for (int i = 0; i < CurrentWeapon->AmmoCount(); i++)
													{
														...
														//{
															...
															PlayerController->InputKey(Keys::LeftMouseButton, EInputEvent::IE_Pressed, 1.f);
															PlayerController->InputKey(Keys::LeftMouseButton, EInputEvent::IE_Released, 0.f);
											...
											{
												if (Settings::Aimbot::PredictProjectiles && CurrentWeapon->IsProjectileWeapon())
												{
													...
													{
														TargetForward = UKismetMathLibrary::GetForwardVector(Variables::GetDamageStartLocation::LastTargetRotation);
													}

...
namespace Render
{
	...
	{
		Variables::DebugCanvasObject->K2_DrawLine(a, b, thickness, color);
	}
	...
	{
		Variables::DebugCanvasObject->K2_DrawText(content, screen, FVector2D(scale, scale), color, scale, center_x, center_y, outlined);
	}
	...
	void box_rounded(FVector2D position, FVector2D size, FLinearColor render_color, float thickness, double rounding, double rounding_step) 
	{
		...
		for (int i = 180; i < 270; i += rounding_step) {
			...
			FVector2D result = FVector2D(rounding * UKismetMathLibrary::cos(radians) + rotate_point.X, rounding * UKismetMathLibrary::sin(radians) + rotate_point.Y);
			if (previous)
				Render::Line(previous, result, render_color, thickness);
			previous = result;
		}
		...
		Render::Line(FVector2D(position.X, position.Y + rounding), FVector2D(position.X, position.Y + size.Y - rounding), render_color, thickness);

		Render::Line(FVector2D(position.X + size.X, position.Y + rounding), FVector2D(position.X + size.X, position.Y + size.Y - rounding), render_color, thickness);

		Render::Line(FVector2D(position.X + rounding, position.Y + size.Y), FVector2D(position.X + size.X - rounding, position.Y + size.Y), render_color, thickness);

		Render::Line(FVector2D(position.X + rounding, position.Y), FVector2D(position.X + size.X - rounding, position.Y), render_color, thickness);
	}
